...
public:
    ...
    void Initialize(const char* name, const char* group,
                    const char* label, const char* helpText, int32_t initialVal,
                    int32_t minVal, int32_t maxVal);

    virtual void Update(const Float4x4& viewMatrix) override;

    int32_t Value() const;
    void SetValue(int32_t newVal);
    operator int32_t();

    int32_t MinValue() const { return minVal; };
    int32_t MaxValue() const { return maxVal; };
    void SetMinValue(int32_t newMinVal);
    void SetMaxValue(int32_t newMaxVal);
...
public:
    ...
    void Initialize(const char* name, const char* group,
                    const char* label, const char* helpText, uint32_t initialVal,
                    uint32_t numValues, const char** valueLabels);

    virtual void Update(const Float4x4& viewMatrix) override;

    uint32_t Value() const;
    void SetValue(uint32_t newVal);
    void ClampNumValues(uint32_t num);
...
public:
                    ...
                    uint32_t numValues_, const char** valueLabels_)
    {
        EnumSetting::Initialize(name_, group_, label_, helpText_,
                                uint32_t(initialVal), numValues_, valueLabels_);
    }
    ...
    {
        EnumSetting::SetValue(uint32_t(newVal));
    }
};
...
public:
    ...
    void Initialize(uint64_t numGroups);
