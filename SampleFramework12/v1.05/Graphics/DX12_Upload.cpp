...
struct UploadQueue
{
    ...
    uint64_t SubmitCmdList(ID3D12GraphicsCommandList* cmdList, bool syncOnDependentQueue)
    {
        AcquireSRWLockExclusive(&Lock);
        ...
        const uint64_t newFenceValue = ++FenceValue;
...
struct UploadRingBuffer
{
    ...
    void Resize(uint64_t newBufferSize)
    {
        Release(Buffer);
    ...
    void ClearPendingUploads(uint64_t waitCount)
    {
        const uint64_t start = SubmissionStart;
        const uint64_t used = SubmissionUsed;
    ...
    void Flush()
    {
        ...
        while(SubmissionUsed > 0)
            ClearPendingUploads(uint64_t(-1));
    ...
    UploadSubmission* AllocSubmission(uint64_t size)
    {
        Assert_(SubmissionUsed <= MaxSubmissions);
        if(SubmissionUsed == MaxSubmissions)
            return nullptr;

        const uint64_t submissionIdx = (SubmissionStart + SubmissionUsed) % MaxSubmissions;
        ...
        if(end < BufferSize)
        {
            const uint64_t endAmt = BufferSize - end;
            if(endAmt >= size)
            {
                allocOffset = end;
            }
    ...
    UploadContext Begin(uint64_t size)
    {
...
static const uint64_t TempBufferSize = 16 * 1024 * 1024;
static ID3D12Resource* TempFrameBuffers[RenderLatency] = { };
static uint8_t* TempFrameCPUMem[RenderLatency] = { };
static uint64_t TempFrameGPUMem[RenderLatency] = { };
static int64_t TempFrameUsed = 0;
...
struct FastUploader
{
    ...
    uint64_t CmdAllocatorIdx = 0;

    static const uint64_t MaxFastUploads = 256;
    FastUpload Uploads[MaxFastUploads];
    int64_t NumUploads = 0;
    ...
    {
        const int64_t idx = InterlockedIncrement64(&NumUploads) - 1;
    ...
    void SubmitPending(UploadQueue& queue)
    {
        ...
        for(int64_t uploadIdx = 0; uploadIdx < NumUploads; ++uploadIdx)
        {
