...
enum class SGSolveMode : uint32_t
{
    NNLS,
    SVD,
    Projection
...
void ProjectOntoSGs(const Float3& dir, const Float3& color, SG* outSGs, uint64_t numSGs);

void SolveSGsForCubemap(const Texture& texture, SG* outSGs, uint64_t numSGs, SGSolveMode solveMode = SGSolveMode::NNLS);
