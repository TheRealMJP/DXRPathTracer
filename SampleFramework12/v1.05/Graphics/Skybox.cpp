...
bool SkyCache::Init(const Float3& sunDirection_, float sunSize, const Float3& groundAlbedo_, float turbidity, bool createCubemap)
{
    ...
    if(createCubemap)
    {
        ...
        const uint32_t NumTexels = CubeMapRes * CubeMapRes * 6;
        ...
        {
            for(uint32_t y = 0; y < CubeMapRes; ++y)
            {
                for(uint32_t x = 0; x < CubeMapRes; ++x)
                {
                    Float3 dir = MapXYSToDirection(x, y, s, CubeMapRes, CubeMapRes);
                    Float3 radiance = Sample(dir);

                    uint32_t idx = (s * CubeMapRes * CubeMapRes) + (y * CubeMapRes) + x;
...
{
    // Load the shaders
    const std::string shaderPath = SampleFrameworkDir() + "Shaders\\Skybox.hlsl";
    ...
    vbInit.Name = "Skybox Vertex Buffer";
