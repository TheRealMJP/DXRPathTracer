...
uint64_t Setting::SerializedValueSize()
{
    ComputeSizeSerializer serializer;
    const uint64_t startSize = serializer.Size();
...
void IntSetting::Initialize(const char* name_, const char* group_,
                            const char* label_, const char* helpText_, int32_t initialVal,
                            int32_t minVal_, int32_t maxVal_)
{
    val = initialVal;
    oldVal = initialVal;
    minVal = minVal_;
...
void IntSetting::SetValue(int32_t newVal)
{
    val = Clamp(newVal, minVal, maxVal);
    changed = oldVal != val;
    oldVal = val;
...
IntSetting::operator int32_t()
{
    return val;
}

void IntSetting::SetMinValue(int32_t newMinVal)
{
    minVal = newMinVal;
    val = Max(val, minVal);
}

void IntSetting::SetMaxValue(int32_t newMaxVal)
{
    maxVal = newMaxVal;
    val = Min(val, maxVal);
}
...
void EnumSetting::Initialize(const char* name_,
                             const char* group_, const char* label_,
                             const char* helpText_, uint32_t initialVal,
                             uint32_t numValues_, const char** valueLabels_)
{
...
{
    ImGui::Combo(label.c_str(), reinterpret_cast<int32_t*>(&val), valueLabels, numValuesClamp);
    if(ImGui::IsItemHovered() && helpText.length() > 0)
        ImGui::SetTooltip("%s", helpText.c_str());
...
void EnumSetting::SetValue(uint32_t newVal)
{
...
void EnumSetting::ClampNumValues(uint32_t num)
{
...
void SettingsContainer::Initialize(uint64_t numGroups)
{
    groups.Init(numGroups);
    initialized = true;
}

void SettingsContainer::Update(uint32_t displayWidth, uint32_t displayHeight, const Float4x4& viewMatrix)
{
    if(opened == false)
    {
        ...
        ImGui::SetNextWindowPos(ImVec2(displayWidth - buttonSize.x * 2.0f, buttonSize.y * 2.0f));
    ...
    for(uint64_t groupIdx = 0; groupIdx < groups.Count(); ++groupIdx)
    {
        ...
        for(uint64_t settingIdx = 0; settingIdx < group.Settings.Count(); ++settingIdx)
        {
